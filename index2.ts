...
interface ISortedByIdObject {
  [key: string]: IPost;
}

interface INormilizedData {
  byId: ISortedByIdObject;
  allIds: Array<string>;
}
...
const normalizeData = (unnormalizedData: Array<IPost>): INormilizedData => {
  const allIdsArray: Array<string> = unnormalizedData.map((el: IPost) => el.id);
  const sortedByIdObject: ISortedByIdObject = {};

  unnormalizedData.forEach((el: IPost) => {
    sortedByIdObject[el.id] = el;
  });

  return { byId: sortedByIdObject, allIds: allIdsArray };
};
